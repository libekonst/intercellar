...
import { createServing, mapServingToEmoji, Serving } from './Serving';
import { Servings } from '../game-preferences/Servings';
import { getRandomPosition, getScreenCenterPosition } from '../utils/utils';
...
export function ServingsHub({ totalServings }: Props) {
  ...
  const addServing = () =>
    setActiveServings((prev) => [...prev, createServing(Servings.LAGER)]);
  ...
  const transitioningServings = useTransition(activeServings, (item) => item.id, {
    from: {
      opacity: 0,
      transform: 'scale(0.5)',
      position: 'absolute',
